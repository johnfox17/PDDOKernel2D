...
class firstOrderXFirstOrderYDerivativePDDOKernel:
    def __init__(self):
        ...
        self.bVec11 = PDDOConstants.BVEC11
    ...
    def calculateGPolynomials(self):
        ...
        for iNode in range(len(self.PDDOKernelMesh)):
            ...
            pList = np.array([1, currentXXi/deltaMag, currentYXi/deltaMag, currentXXi*currentYXi/deltaMag**2])
        ...
        for iNode in range(len(self.PDDOKernelMesh)):
            ...
            pList = np.array([1, currentXXi/deltaMag, currentYXi/deltaMag, currentXXi*currentYXi/deltaMag**2])
            weight = np.exp(-4*(xiMag/deltaMag)**2)
            g11.append((self.dx*self.dy/(np.sqrt(2)*self.horizon*self.dx))*weight*(np.inner(solve(diffMat,self.bVec11), pList)))
        
        self.g11 = np.array(g11).reshape((self.kernelDim,self.kernelDim))
        self.g11[np.absolute(self.g11)<10**-9]=0
